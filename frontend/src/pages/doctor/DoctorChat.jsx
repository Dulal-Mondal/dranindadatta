...
//         socket.on('user_typing', ({ senderId }) => { if (senderId === patientId) setTyping(true); });
//         socket.on('user_stop_typing', ({ senderId }) => { if (senderId === patientId) setTyping(false); });
//         return () => { socket.off('receive_message'); socket.off('message_sent'); socket.off('user_typing'); socket.off('user_stop_typing'); };
//     }, [socket, patientId]);
...
//             const p = res.data.users?.find((u) => u._id === patientId);
//             setPatient(p);
...
//     const handleSend = () => {
//         if (!text.trim()) return;
//         socket.emit('send_message', { senderId: user._id, receiverId: patientId, text: text.trim() });
//         socket.emit('stop_typing', { senderId: user._id, receiverId: patientId });
//         setText('');
//     };

//     const handleTyping = (e) => {
//         setText(e.target.value);
//         socket.emit('typing', { senderId: user._id, receiverId: patientId });
//         clearTimeout(typingTimeoutRef.current);
//         typingTimeoutRef.current = setTimeout(() => socket.emit('stop_typing', { senderId: user._id, receiverId: patientId }), 1500);
//     };
...
//                     <Link to="/doctor/appointments" className="text-gray-400 hover:text-gray-600"><FiArrowLeft size={20} /></Link>
//                     <div className="relative">
//                         <img
//                             src={'https://ui-avatars.com/api/?name=' + (patient?.name || 'P') + '&background=6366f1&color=fff&size=80'}
//                             className="w-10 h-10 rounded-full object-cover"
//                             alt={patient?.name}
//                         />
//                         {isOnline(patientId) && <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-full border-2 border-white" />}
//                     </div>
//                     <div>
//                         <p className="font-semibold text-gray-800 text-sm">{patient?.name || 'Patient'}</p>
//                         <p className="text-xs text-gray-500">{isOnline(patientId) ? 'Online' : 'Offline'}</p>
//                     </div>
//                 </div>
...
//                                     {msg.text && (
//                                         <div className={'px-4 py-2.5 rounded-2xl text-sm max-w-xs lg:max-w-md ' + (isMe ? 'bg-primary-500 text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm')}>
//                                             {msg.text}
//                                         </div>
//                                     )}
...
//                     />
//                     <button onClick={handleSend} disabled={!text.trim()} className="w-10 h-10 bg-primary-500 hover:bg-primary-600 disabled:opacity-50 text-white rounded-xl flex items-center justify-center transition">
//                         <FiSend size={16} />
//                     </button>
//                 </div>
//             </div>
    ...
    const fetchPatient = async () => {
        try {
            ...
            const conv = data.conversations?.find((c) =>
                c.participants?.some((p) => p._id === patientId || p === patientId)
            );
            if (conv) {
                const p = conv.participants?.find((p) => p._id === patientId || p === patientId);
                setPatient(p);
            }
        } catch (err) { console.error(err); }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex-1 max-w-3xl w-full mx-auto px-4 py-4 flex flex-col" style={{ height: 'calc(100vh - 64px)' }}>
                ...
                {/* Header */}
                <div className="bg-white rounded-t-xl border border-gray-100 p-4 flex items-center gap-3">
                    <Link to="/doctor/messages" className="text-gray-400 hover:text-gray-600"><FiArrowLeft size={20} /></Link>
                    <div className="relative">
                        <img
                            src={patient?.avatar || 'https://ui-avatars.com/api/?name=' + (patient?.name || 'P') + '&background=6366f1&color=fff&size=80'}
                            className="w-10 h-10 rounded-full object-cover"
                            alt="patient"
                        />
                        {isOnline(patientId) && <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-full border-2 border-white" />}
                ...
                <div className="flex-1 bg-white border-x border-gray-100 overflow-y-auto p-4 space-y-3">
                    {loading ? (
                        ...
                        <p className="text-center py-10 text-gray-400 text-sm">No messages yet.</p>
                    ) : (
                        messages.map((msg, i) => {
                            const isMe = msg.sender?._id === user._id || msg.sender === user._id;
